...
!	!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	Pbetatemp=Pbeta  
    deltat = dt  !设置固体推进时间步deltat为流体时间步dt 
!	===============================================================================================
...
!	===============================================================================================
    CALL cptMacr()
    CALL wrtFlow(1)
    CALL wrtBody(xyzful/Lref,velful/Uref,accful/Aref,extful/Fref,ele,time/Tref,nND,nEL,0)
    CALL wrtImag() 
!==================================================================================================
!==================================================================================================
!==================================================================================================      
    ...
    do while(time/Tref < timeSimTotl)
        ...
        write(*,*)'time for streams:',(values1(6)*60.+values1(7)*1.+values1(8)*0.001)-(values0(6)*60.+values0(7)*1.+values0(8)*0.001)
		!******************************************************************************************
        ...
        CALL cptForcS(zDim,yDim,xDim,nEL,nND,ele,dx,dy,dz,dh,Uref,denIn,dt,uuu,den,xGrid,yGrid,zGrid,  &
...
!        CALL cptStrs(zDim,yDim,xDim,nEL,nND,ele,dh,dx,dy,dz,mu,2.50d0,uuu,prs,xGrid,yGrid,zGrid,xyzful,extful2)
        ...
        CALL collide()
        call date_and_time(VALUES=values1)
        write(*,*)'time for collide:',(values1(6)*60.+values1(7)*1.+values1(8)*0.001)-(values0(6)*60.+values0(7)*1.+values0(8)*0.001)
        !给固体有限元的流体作用力：两种：罚力和流体应力
...
!        endif                           
        ...
        if(iBodyModel==1)then     ! 刚性体模型
            ...
            enddo
            !-------------------------------------------------------
        elseif(iBodyModel==2)then !弹性体模型          
            !平动位移     
            call date_and_time(VALUES=values0)              
            XYZ(1:3)=XYZo(1:3)+XYZAmpl(1:3)*dcos(2.0*pi*Freq*(time-deltat+isubstep*subdeltat)+XYZPhi(1:3))
            !转动位移
		...
		if(DABS(time/Tref-timeOutBody*NINT(time/Tref/timeOutBody)) <= 0.5*dt/Tref)then
            ...
            CALL wrtBody(xyzfulIB/Lref,velful/Uref, accful/Aref,extful/Fref,ele,time/Tref,nND,nEL,1)
            endif
            endif
		endif
        ...
        !if(DABS(time/Tref-timeOutBody*NINT(time/Tref/timeOutBody)) <= 0.5*dt/Tref .and. time/Tref>=timeOutFlBg .and. time/Tref<=timeOutFlEd)then
            !CALL wrtSlic(xDim,yDim,zDim,XGrid,yGrid,zGrid,Lref,Uref,denIn,prs,uuu,time/Tref-timeOutFlBg,3)
		!endif
